...
\begin{titlepage}
  \centering
  \vspace*{4cm}
  {\LARGE\textbf{Capstone: Ovarian Cancer Prediction Simulation}\\[1.5em]}
...
% Local redefinition to handle missing experiment images gracefully
\let\oldincludegraphics\includegraphics
\renewcommand{\includegraphics}[2][]{%
  \IfFileExists{#2}{%
    \oldincludegraphics[#1]{#2}%
  }{%
    \framebox[\linewidth]{\tiny Missing: \detokenize{#2}}%
  }%
}

\subsection{Per-Model Accuracy and Loss Curves}
\label{appx:acc-loss}
\input{sections/appx-acc-loss}

\subsection{Per-Model AUC-ROC Curves}
\label{appx:auc-roc}
\input{sections/appx-auc-roc}

\subsection{Per-Model Confusion Matrices}
\label{appx:confusion}
\input{sections/appx-confusion}

\let\includegraphics\oldincludegraphics % Restore normal behavior

\input{sections/appx-per-arch-results}

\subsection{Complete 72-Run Results Table}
\label{appx:full-results}
\input{sections/appx-full-results}

\subsection{Source Code}
\label{appx:source-code}
\input{sections/appx-source-code}
