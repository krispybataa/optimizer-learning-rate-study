...
The design of the classification head (Table~\ref{tab:head}) was provided in the project specifications. It uses a series of Dense layers with Batch Normalization and Dropout to prevent overfitting on our relatively small dataset. We used GlobalAveragePooling2D to bridge the gap between the frozen base and our dense layers, a pattern we adapted from our course materials.

\begin{table}[H]
  \centering
  \caption[Classification Head]{Our standard classification head (used for all 72 runs).}
  \label{tab:head}
  \begin{tabular}{lll}
    \toprule
...
\begin{longtable}{r l l}
  \caption[Experiment Matrix]{The full 72-run experiment matrix.}
  \label{tab:runs} \\
  \toprule
  \rowcolor{tableheader}
