...
Overall, the 72 runs showed a wide range of results. We observed everything from near-perfect classification to complete model failure. The best performance came from VGG19 using Adam and Adamax at a learning rate of $10^{-4}$, both reaching an AUC of 0.9995. On the other end of the spectrum, we had 10 subopotimal runs where the model failed to learn anything useful and just predicted one class for every image.

\subsection{Top Performers}

When we look at the best model for each architecture (Table~\ref{tab:best-arch}), we see that every base model is capable of high performance if given the right optimizer. VGG19 and DenseNet121 both hit very high AUC scores with Adam, while ResNet50 and EfficientNetB3 performed best with RMSProp. The full suite of resultes are documented in Appendices Sections~\ref{appx:per-arch-results} and~\ref{appx:full-results}.
...
\subsection{Failures and Suboptimal Runs}
